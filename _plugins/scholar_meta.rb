...
module ScholarMeta
  ...
  BUTTON_ICON_CSS = <<~'CSS'.freeze
    <style>
      ...
      /* WCAG 1.4.3: the theme's muted-text colour is #828282 in both schemes,
         which is 3.84:1 on white and 4.43:1 on the dark background -- both under
         the 4.5:1 minimum. Override the variable rather than one class, so post
         meta, post tags and the author overflow text are all fixed together.
         Matching the theme's own selectors (:root and html[data-theme=dark]) is
         what makes these win; a plain .more-authors rule loses to the theme's
         `.publications ol.bibliography li .author>span.more-authors`. */
      :root {
        --global-text-color-light: #6e6e6e;
      }
